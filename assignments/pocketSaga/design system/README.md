...
| Material | Neutral, mint and strong-mint surfaces; Container/Pill geometry; cumulative construction steps |
| Background | Artwork → Blur → Mute → Palette → Contrast; fixed sample text demonstrates readability |
| Shapes | Rounded rectangles for content/writing; pills for actions/selections; circles for identity/icon controls; real Dark discussion copy |
| Color | Foundation, Text, Primary accent, Secondary accent & feedback; source hex values from the design brief |
...
- Material construction steps sit directly below the three samples inside `main` (72px gap); Background steps likewise sit directly below the sample text. Both have with no description line beneath them (the step description text is hidden). Shape toggle bottom-left; artwork arrows, thumbnail and counter bottom-right.
...
- Material and Background use the blurred banner atmosphere and show the artwork selector. Shapes, Color, Spacing, Typography and Avatars use the animated Ribbon flow shader instead and hide the selector.
- Scroll-connected tabs: scrolling (wheel/trackpad, vertical) past the end of a view's content by 140px of overscroll slides the view up and out (260ms, fading) and slides the next sidebar view in from below (380ms); scrolling up past the top goes to the previous view and lands at its bottom. The sidebar selection follows. A 320ms quiet-wheel cooldown absorbs trackpad momentum so one flick advances one view. The title view moves its fixed stage directly. Reduced motion switches without the slide. Wheel events over the sidebar are ignored.
- Keyboard: ArrowUp/ArrowDown (and Page Up/Down) scroll the pane's content and, at its edge, move to the previous/next view with the same slide (ignored inside the sidebar list, which keeps its own up/down focus navigation). The slide moves the whole right pane — its glass fill and rim (`body::before`/`::after`, animated as pseudo-elements) — together with the content.
...
2. Tint: neutral remains 4% white; mint uses `rgb(178 219 194 / 14%)`; strong mint uses `rgb(128 213 169 / 25%)`. These are alternative fills, not stacked opacity percentages.
3. Wash & blur: directional gradient over the fill; 18px backdrop blur for all three samples; strong mint also uses 1.25 saturation.
4. Edge: masked 1px directional rim; strong mint includes its inset and small outer shadows.
...
Blur softens content behind the material, not the wash. It is barely visible over an already blurred wallpaper and invisible over a uniform base. Do not add a blur slider: the demo intentionally uses a fixed 18px material value. The app itself still has multiple surface blur values; this is not a claim of global uniformity.
